...
const Page = () => {
  ...
  const handleHasil = (e) => {
    if (e >= 1 && e <= 5) {
      setHasilAkhir((e * (10 / 100)).toFixed(2))
    } else {
      setHasilAkhir("error")
    }
    setHasilMor(e)
  ...
  const getTingkatKehadiran = async (page, size, month, year, search_name) => {
    let dataUser
    await axios.get(`${config.baseURL}/api/serve-data/disiplin-kerja?page=${Number(page) + 1}&size=${size}&month=${month}&year=${year}&search_name=${search_name}`).then((res) => {
      dataUser = res
  ...
  return (
    <>
      <Head>
        <title>
          Tepat Waktu dan Disiplin Kerja  | MOR Recap
        </title>
      </Head>
      <Box
        component="main"
